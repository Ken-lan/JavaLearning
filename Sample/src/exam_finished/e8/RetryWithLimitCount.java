...
public class RetryWithLimitCount {
	public static void main(String[] args) {
		 ...
		 * 最後將取得的結果以及呼叫次數列印出來 2.catch exception的block，不能再寫try catch，例如
		 * catch(IllegalStateException ex){ try {} catch.... } 例如： 第1次取得 3 呼叫2次 第2次取得
		 * 無法取得 第3次取得 4 呼叫1次 .. 第10次取得 5
		 */
		...
		for (int i = 0; i < 10; i++) {
			try {
				...
				int j = 0;
				for (j = 0; array[i] == 0 && j<3 ; j++) {

					random = getRandomDouble() * 10;
					array[i] = (int) random;

					
				}
				System.out.printf("第%d次取得%d呼叫%d次%n", i + 1, array[i], j);

			} catch (IllegalStateException e) {

				System.out.printf("第%d次無法取得%n", i + 1);
